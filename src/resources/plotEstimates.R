...
t_mapFastq <- filter(t_mapFastq, Condition != "Spt6+.2p", .preserve = T)
...
t_resc <- filter(t_resc, Condition != "Spt6+.2p", .preserve = T)

t_merge <- rbind(t_mapFastq,t_resc)
t_merge=t_merge[!grepl("no4sU",t_merge$Condition),]
t_merge <- filter(t_merge, Condition != "Spt6+.0p", .preserve = T)
