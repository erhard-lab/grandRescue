...
cp {bampath} .
gedi -e ExtractUnmappedReads {tags} -strandness {strandness} -f {prefix}.bam
...
gedi -e RescuePseudoReads -genome {genome} -pseudogenome {pseudogenome} -origmaps {prefix}_final.bam -pseudomaps {prefix}_pseudoMapped.bam -strandness {strandness}
